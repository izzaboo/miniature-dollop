...
// Required (header) data : token (a.k.a. id)
// Optional data : none
handlers._tokens.delete = function(data,callback){
  // Check that the id is valid.
  var id = typeof(data.headers.id) == 'string' && data.headers.id.trim().length == 20 ? data.headers.id.trim() : false;
  var usereMail = typeof(data.queryStringObject.email) == 'string' && data.queryStringObject.email.trim().indexOf('@') > 0 && data.queryStringObject.email.trim().indexOf('.') > 0 ? data.queryStringObject.email.trim() : false;
  if(id && usereMail){
    // Lookup the user
    _data.read('tokens',id,function(err,userData){
      if(!err && userData.email === usereMail){
        ...
        callback(400,{'error':'The specified token does not exist or does not match user'});
      }
    });
  } else {
    callback(400,{'error':'Missing the required field'});
...
// orders - post
// Required headers: token
// Required (payload) data: drinks, entrees, deserts (one must be sent)
// Optional data: none
handlers._orders.post = function(data,callback){
  // Get order from input
  var drinks = typeof(data.payload.drinks) == 'object' && data.payload.drinks instanceof Array && data.payload.drinks.length > 0 ? data.payload.drinks : false;
  var entrees = typeof(data.payload.entrees) == 'object' && data.payload.entrees instanceof Array && data.payload.entrees.length > 0 ? data.payload.entrees : false;
  var deserts = typeof(data.payload.deserts) == 'object' && data.payload.deserts instanceof Array && data.payload.deserts. length > 0 ? data.payload.deserts : false;
  if(drinks || entrees || deserts){
    // Get the token from the headers
...
// Orders - put
// Required (query) data: orderid
// Required (headers) data: token
// Optional (body) datapoints: drinks, entrees, deserts. (one, as Array, must be sent)
handlers._orders.put = function(data,callback){
  var id = typeof(data.queryStringObject.orderid) == 'string' && data.queryStringObject.orderid.trim().length == 20 ? data.queryStringObject.orderid.trim() : false;
        ...
        if(!err && orderData){
          ...
          handlers._tokens.verifyToken(token,orderData.usereMail,function(tokenIsValid){
            if(tokenIsValid){
              ...
              if(drinks){
                orderData.drinks = drinks;
              }
              if(entrees){
                orderData.entrees = entrees;
              }
              if(deserts){
                orderData.deserts = deserts;
              }
          ...
          handlers._tokens.verifyToken(token,orderData.usereMail,function(tokenIsValid){
            if(tokenIsValid){
              ...
              _data.delete('orders',id,function(err){
                if(!err){
                  // Lookup the user
                  _data.read('users',orderData.usereMail.replace('@','-+-'),function(err,userData){
                    if(!err && userData){
                      var userOrders= typeof(userData.orders) == 'object' && userData.orders instanceof Array ? userData.orders : [];
                      ...
                      if(orderPosition > -1){
                        ...
                        _data.update('users',orderData.usereMail.replace('@','-+-'),userData,function(err){
                          if(!err){
                            callback(200);
                          } else {
                        ...
                        callback(500,{'error':'Could not find the order on the users object so could not remove it.'});
                      }
                    } else {
                      callback(500,{'error':'Could not find the user who created the order. Could not remove the order from the list of orders on the user object.'});
                    }
                  });
                } else {
